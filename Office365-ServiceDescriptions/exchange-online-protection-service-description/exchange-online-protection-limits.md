...
- **Limite de domaines distants** : vous pouvez ajouter jusqu’à 200 domaines distants par client.
    
- **Limite de taille des messages** -la taille maximale des messages pour les clients autonomes EOP, y compris les pièces jointes, est de 150 Mo. 
    
- **Nombre de messages sortants envoyés** : la limite pour le nombre de messages sortants envoyés via EOP est suffisamment élevée pour garantir que la communication de messagerie normale n’est pas traitée comme du courrier indésirable. Si vous voulez envoyer des messages électroniques commerciaux en bloc, plutôt que d'envoyer des messages sortants via EOP, nous vous recommandons d'utiliser un fournisseur de services de messagerie tiers (ESP) ou de les envoyer via vos serveurs de messagerie sur site. 
    
- **Limite des destinataires** -tant que l’hôte expéditeur peut fractionner le message en « segments » de moins de 500 destinataires, aucune limite explicite n’est définie. Toutefois, chaque « segment » est traité comme un nouveau message. Les messages trop nombreux sur une courte période, les messages provenant d'un hôte avec une mauvaise réputation ou les messages avec un contenu douteux pourraient être limités ou bloqués. 
    
- Limite de liste d’adresses IP **autorisées ou bloquées** -lors de la configuration d’une liste d’adresses IP autorisées ou d’une liste d’adresses IP bloquées dans le filtre de connexion, vous pouvez spécifier un maximum de 1273 entrées, où une entrée est une adresse IP unique ou une plage CIDR d’adresses IP de/24 à/32. 
    
- **Limite de report de message** : les messages en différé resteront dans nos files d’attente pendant 24 heures. Les nouvelles tentatives d'envoi de message sont basées sur le type d'erreur reçu à partir du système de messagerie du destinataire. Les messages sont renvoyés toutes les 15 minutes. 
...
- **Notifications de mise en quarantaine du courrier indésirable de l’utilisateur final** : par défaut, si ce paramètre est activé, les notifications de quarantaine de courrier indésirable sont envoyées tous les 3 jours. Il est possible de configurer ces notifications afin qu'elles soient envoyées tous les 1 à 15 jours. 
    
- **Limites de suivi des rapports et de suivi** des messages : pour les limites de création de rapports et de suivi des messages, consultez la section « disponibilité et latence des données de suivi des rapports et des messages » dans la rubrique [Reporting and message trace in Exchange Online Protection](https://go.microsoft.com/fwlink/?LinkId=394248).
